...
backgroundBinomial <- function(maf = NULL, coverage = NULL, covariate = NULL, original_mutation_rate = 1.2e-6,
                               ref_genome = NULL, category_num = NULL){
  options(digits=15)
  ...
  if(!"categ" %in% colnames(maf)){
    Categ_result <- mafCategoryAssign(maf, coverage, ref_genome = ref_genome,
                                      category_num = category_num, output_file = FALSE, quiet = TRUE)
  ...
  covariate <- covariatePreprocess(covariate, maf = maf, output_file = FALSE, quiet = TRUE)
  ...
  if(tot_n_noncoding==0 || tot_n_noncoding<min_tot_n_noncoding ||
     tot_rate_noncoding < min_rate_noncoding || tot_rate_noncoding > max_rate_noncoding ||
     abs(log2(tot_rate_noncoding/tot_rate_coding)) > max_abs_log2_difference_noncoding_coding){
    ok = FALSE
  }else{
    ok = TRUE
  }
  ...
  analysis_gene <- as.character(analysis_gene)
  return(analysis_gene)
}
