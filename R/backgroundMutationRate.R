...
backgroundMutationRate <- function(maf = NULL, coverage = NULL, covariate = NULL,
  ...
  G1 <- plyr::arrange(G1,plyr::desc(rate))
  ...
  if(output_file){
    ...
    bmr_pic <- ggplot2::ggplot(G1,ggplot2::aes(x=Hugo_Symbol,y=rate))+
      ggplot2::geom_bar(stat = "identity", fill = "#00AFBB") +
    ...
    ggplot2::ggsave(bmr_pic, file="background_mutation_rate.pdf")
