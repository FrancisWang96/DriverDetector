...
# fill missing data in covariate file 2019/4/8 wang
covariate_preprocessing <- function(M,V){
  ...
  geneEffect1 <- as_tibble(geneEffect)
  catgories <- geneEffect1 %>%
    group_by(silent,nonsilent,noncoding,null) %>%
    summarise(n = n())
  k <- round(nrow(catgories)/50)
  km_result <- kmeans(geneEffect, k, nstart = k,iter.max = 100)
  # print(km_result)
  ...
  for(i in 1:k){
    ...
    flag<-flag[complete.cases(flag)]
    means[i,2] <- mean(V1$expr[flag])

    V1 <- subset(V,!is.nan(reptime))
    flag <- match(select1$gene,V1$gene)
    flag<-flag[complete.cases(flag)]
    means[i,3] <- mean(V1$reptime[flag])

    V1 <- subset(V,!is.nan(hic))
    flag <- match(select1$gene,V1$gene)
    flag<-flag[complete.cases(flag)]
    means[i,4] <- mean(V1$hic[flag])
  }
  ...
  V <- V%>%
    filter(!is.na(expr),!is.na(reptime),!is.na(hic))
  return(V)

}

# plotEffect <- function(M){
...
#
#   return(fig_effect)
#   # ggplot2::ggsave(pic2, file="EffectPlot.pdf", width=8, height=8)
# }
#
...
#
#   return(fig_category)
#   # ggplot2::ggsave(pic1, file="CategoryPlot.pdf", width=8, height=8)
#
# }
