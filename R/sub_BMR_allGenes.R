...
#' @param preOutV Covariate data which includes expression level, replication time and chromatin
#' compartment defaultly, and could be altered by users.
#' @param bmr The default background mutation rate is 1.2e-6, and the value alters when
#' function ends.
#' @return The output BMR_out is an input to the sigGenes method or projection method
#' @export
...
BMR <- function(preOutM, preOutC, preOutV, bmr=1.2e-6)
{
    ...
    if (!ok){
        print('Zeroing out all noncoding mutations and coverage for the rest
              of the calculation.\n');
        n_noncoding[,,]= 0;
        N_noncoding[,,] = 0;
    }
    ...
    for (g in 1:length(analysis_gene)){
        if (g %% 1000 == 0) print(g)
        gi <- which(G$gene %in% analysis_gene[g])
    ...
    return(list(G=G,M=M,N_silent=N_silent,n_silent=n_silent,
    ...
    # write.table(G,file = paste(output_filestem,"_G.txt",sep = ""),
