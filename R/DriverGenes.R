...
#' Test, Fisher’s Combined p-value Test, Likelihood Ratio Test, Convolution Test, and Projection method. The output of `DriverGenes` includes preprocessed results and significant genes,
...
#'
#' @param Mutation Mutation maf data, default to be NULL.
#' @param Coverage Coverage raw data, default to be NULL.
#' @param Covariate Covariate data which includes expression level, replication time and chromatin
#' compartment defaultly, and could be altered by users, default to be NULL.
#' @param MutationDict Mapping Variant_Classification to mutation effect in mutation data, default to be NULL.
#' @param chr_files_directory Chromosome files directory, "hg19" or "hg38".
#' @param categ_flag Mutation category number, should be either NaN or numeric, default to be NULL, maximum to be 6.
...
#' @param bmr The initial background mutation rate is 1.2e-6, and the value alters when
#' function ends.
#' @param p_class Method of hypothesis. "BB" represents Beta Binomial test, "FCPT" represent Fisher combined P-value test,
...
#' @param file_stem Unified prefix of output data, could be modified.
#'
#' @return A list of significant genes.
#'
#'
#' @export
DriverGenes <- function(Mutation=NULL,Coverage=NULL,Covariate=NULL,MutationDict=NULL,
                        chr_files_directory="chr_files_hg19",categ_flag=NaN, bmr=1.2e-6,
                        p_class="allTest", sigThreshold = 0.05, file_stem="text"
)
{
  if(!is.null(Mutation) & !is.null(Coverage) & !is.null(Covariate) & !is.null(MutationDict)){
    preOut <- preprocessing(Mutation, Coverage, MutationDict,Covariate,
                            chr_files_directory,categ_flag,output_filestem=file_stem)
    plotCategory(preOut$M)
    plotEffect(preOut$M)
    setwd("..")

    BMR_out <- BMR(preOut$M, preOut$C, preOut$V, bmr)
    sigGenes(BMR_out, p_class,output_filestem=file_stem,sigThreshold)
  }
  else cat("Error:invalid input!")
}
